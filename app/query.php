...
const QUERIES = [
    ...
    "get_feed_posts" => "SELECT * FROM post p JOIN post_like pl ON p.post_id = pl.post_id WHERE p.username IN (SELECT to_username FROM `following` WHERE from_username = ?) ORDER BY p.`timestamp` DESC",
